...
```{r}
sce <- list(
  EB_KDR_HET_1 = readRDS("data/EB_KDR_HET_1.rds"),
  EB_KDR_HOM_2 = readRDS("data/EB_KDR_HOM_2.rds")
)
```
...
```{r}
dec <- list(
  EB_KDR_HET_1 = metadata(sce$EB_KDR_HET_1)$modelGeneVarByPoisson,
  EB_KDR_HOM_2 = metadata(sce$EB_KDR_HOM_2)$modelGeneVarByPoisson
)
```
...
```{r}
ind <- c("EB_KDR_HET_1", "EB_KDR_HOM_2")
```
...
```{r}
tab <- table(Cluster = sce$cluster.temp, Batch = mnn$batch)
