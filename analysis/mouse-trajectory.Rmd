...
knitr::opts_chunk$set(
  autodep    = TRUE,
  cache      = TRUE,
  cache.path = "cache/mouse-trajectoryRmd/",
  dev        = "png",
  error      = FALSE,
  message    = FALSE,
  warning    = FALSE,
...
Plot UMAP of kNN cells coloured by celltype:
...
Plot UMAP of kNN cells coloured by genotype:

```{r}
ggplot(dat, aes(UMAP.1, UMAP.2, colour = genotype)) + 
  geom_point(size = 0.1) + 
  scale_colour_manual(
    name = "Genotype",
...
```{r}
use <- sce$celltype %in% c("Allantois",  "ExE mesoderm", "Haematoendothelial progenitors", "Endothelium")
...
sce <- runDiffusionMap(sce, dimred = "corrected", n_dimred = 1:10)

plotDiffusionMap(sce, colour_by = "celltype")
```
...
```{r}
saveRDS(sce, file = "output/mouse-trajectory-endothelium.rds")
```
